...
public class TCPServerSide {
    ...
    public TCPServerSide(TCPServerUser s, int port, int bufferSize) {
        PORT = port;
...
//                TCPServerSide.this.server.bind(this.isa, BACKLOG);
            ...
            Log.i("TCP", "Start waiting for connection.");
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    ...
                    new Thread(new ClientConnection(socketHolder)).start();
    ...
    private class ClientConnection implements Runnable {
        ...
        private ClientConnection(SocketHolder socketHolder) {
...
//                    Log.i("Bytes Read", "" + read);
                    ...
                    if (read >= 1) {
                        bundle.putByteArray(MESSAGE_KEY, Arrays.copyOf(buffer, read));
                        msg.setData(bundle);
                    } else {
